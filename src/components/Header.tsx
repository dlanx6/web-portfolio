...
const Header = () => {
  ...
  return (
    <motion.header
      className={cn("sticky top-0 z-50 transition-all duration-300", isMenuOpen ? "bg-background/90" : "", )}
      initial={{ y: -80 }}
      animate={{ y: 0 }}
    >
      <nav className="container max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <motion.a
          href="#"
          className="text-primary font-mono font-bold text-xl hover:opacity-85 transition-opacity"
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="absolute inset-x-0 md:hidden bg-background/90 backdrop-blur-md border-b border-border/50"
          >
